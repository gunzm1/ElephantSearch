...
function install_nginx() {
  ...
  ln -sf /vagrant/tools/vagrant/nginx/traveling-owl.conf /etc/nginx/sites-enabled/001-traveling-owl.conf
...
function setup_nginx_ssl() {
  ...
  openssl req -new -key server.key -out server.csr -subj "/C=CH/ST=Bern/L=Bern/O=traveling-owl/CN=traveling-owl.vm"
...
function install_traveling-owl() {
  cd /vagrant
