...
Um diese Konklusion zu belegen soll ein Widerspruchsbeweis angewendet werden. Dazu wird die Negation der Folgerung (also SeilparkBalmberg hatRegion Solothurn) hinzugefügt. Konkret heisst das also es wird versucht mit den vorhandenen Fakten \textit{SeilparkBalmberg hat den Standort Balmberg} und \textit{Solothurn (welches eine Region ist) hat den Ort Balmberg} zu beweisen, dass der Seilpark \textbf{NICHT} in der Region Solothurn liegt. Bei diesem simplen Beispiel ist für den Leser sichtbar, dass dies zu einem Widerspruch führen wird. Damit kann die ursprüngliche Annahme bestätigt werden. Durch Anwendet des Tableau-Kalkül kann dies aber auch mathematisch bewiesen werden:
...
Der linke Teilbaum ist somit vollständig expandiert und wird ausgewertet. Dabei werden die Knoten (b), (c), (d), (g) und (h) (logisch) vereint \textcircled{$\cup$}. Dabei ergibt sich,  dass die Knoten (g) und (h) einen Widerspruch mit den Knoten (b) und (c) bilden:
\begin{itemize}
