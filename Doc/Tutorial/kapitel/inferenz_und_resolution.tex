...
Die Anwendung des Tableau-Kalküles läuft folgendermassen ab:
%Der generelle Prozess (der Folgerung TODO: wieso der Folgerung??)  läuft wie folgt ab:
\begin{itemize}
    \item Umwandlung der Konzepte der Abox in die negierte Normalform (NNF) (die Negation $ \neg $ befindet sich vor Konzeptnamen.)
    \item Ablegen der umgewandelten Konzepte als $ C_0 $.
        Der Algorithmus hat also die Datenbasis $ A_0 = {C_0(x_0)} $
    \item Anwendung der Transformationsregeln auf die Datenbasis.
    \item Tritt bei der Anwendung der Transformationsregeln auf $C_0$ ein Widerspruch auf, ist die Negation nicht erfüllbar.

    $\Rightarrow$ Damit sind die ursprünglichen Konzepte der Abox erfüllbar.
\end{itemize} (vgl.~\cite{horrocks2012})

Um die Anwendung des Tableau-Kalküles zu verdeutlichen, sollen die Fakten der Instanz ``Sole Uno'' (welches Sauna und Dampfbad bietet) auf nachfolgende Regel angewendet werden. Dabei soll geprüft werden, dass ``Sole Uno'' Wellness bietet.

$bietet(?x, ``Sauna''), bietet(?x, ``Dampfbad'') \rightarrow wellness(?x, true)$
