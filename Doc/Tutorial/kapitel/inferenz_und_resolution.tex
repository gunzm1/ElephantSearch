...
Der Parser prüft beim Laden einer Ontologie ihre Kompatibilität bezüglich OWL-DL.\@ Über den Parser werden Klassenaxiome in der Tbox, Aussagen über Individuen in der Abox abgelegt. Die Axiome der Tbox ihrerseits werden durch das Standardverfahren von OWL-DL-Reasonern vorbereitet. Sie werden normalisiert, absorbiert, internationalisiert und anschliessend in den Tableaux-Reasoner eingespiesen.
...
Beginnend mit einem initalen Graphen der Abox, sucht der Tableaux-Reasoner durch Vervollständigung des Graphen nach einer Interpretation (Modell der Ontologie). Der Graph wird inkrementell als eine Art Tafel (eben, Tableau) aufgebaut. Dabei repräsentieren Knoten die Individuen und die Literale (z.B. Zeichenketten, Datumswerte oder auch Nummern). Jedem Knoten wird der entsprechende (Daten-) Typ zugewiesen. Die gerichteten Kanten zwischen den Knoten stellen die Eigenschaften dar.

Durch wiederholtes Anwenden der Regeln zur Erweiterung des Graphen, versucht der Reasoner einen widerspruchsfreien Graphen zu bilden. Dies führt er solange aus, bis ein Widerspruch (Kontradiktion) auftritt oder keine Regeln mehr anwendbar sind.
...
findet der Prozess der Folgerung (Modus ponens) wie folgt statt:
\begin{itemize}
    ...
    $ \Rightarrow $ Konflikt!
\end{itemize}
Daraus ist ersichtlich, das Konzept $ C_0 $ ist nicht erfüllbar. Die Anfrage $ Prolog \subseteq Programmiersprache $ trifft somit innerhalb der gegebenen Ontologie zu.
...
Der generelle Prozess der Folgerung läuft ab wie folgt:
\begin{itemize}
...
\end{wrapfigure}
Das unter~\ref{lst:reasoning_seilpark} genannte Beispiel beantwortet exakt die unter~\ref{sec:wissensrepFormen_Wissensnetze} gestellte Frage: Wie gelangt man zum Schluss, dass das Individuum \textit{Seilpark Balmberg} in der \textit{Region} \textit{Solothurn} liegt.
