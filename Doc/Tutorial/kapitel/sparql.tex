...
\lstset{language=XML}
%%% \begin{lstlisting}
%%%     $\textless$http://example.org/book/book1$\textgreater$ $\textless$http://purl.org/dc/elements/1.1/title$\textgreater$ "SPARQL Tutorial".
%%% \end{lstlisting}
...
%%%     WHERE
%%%     {
%%%       $\textless$http://example.org/book/book1$\textgreater$ $\textless$http://purl.org/dc/elements/1.1/title$\textgreater$ ?title .
%%%     }
%%% \end{lstlisting}
\footnotetext{\cite[2.1 Writing a Simple Query]{w3sparql_querylang}}
...
%%%%%      \end{wrapfigure}
%%%%%      Wie bereits beschrieben, sind alle Komponenten vorhanden, um Ausflüge abbilden zu können, welche den unter~\hyperref[elephant_graph_data]{\nameref{chap:graph_data}} genannten Kriterien entsprechen. Mit diesem Kapitel ist nun auch noch die fehlende Abfragesprache um Abfragen in der Ontologie vornehmen zu können eingeführt worden.
%%%%%      
